module Reference
	class CreateRefCode < Core::BaseService
		...
		def process
			begin
				new_code = RefCodes.new(@code)
				new_code.ref_type = @ref_type
				new_code.corporate_id = @context.corporate_id
				new_code.establishment_id = @context.establishment_id
				new_code.save
