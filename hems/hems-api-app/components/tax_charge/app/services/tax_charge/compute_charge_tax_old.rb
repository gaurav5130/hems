module TaxCharge
	class ComputeChargeTaxOld < Core::BaseService
		...
		attribute :order_category_list, Array[OrderItemCategoryVO]
		...
		def validate()
			valid? && (order_category_list.size() > 0)
		end
		...
		def compute_facility_charge_and_tax()
			...
			if facility_charge_exists
				p "computing bill category_total"
				p @result_vo.charges.tot_amt
				compute_tax(@charge_tax_code, @result_vo.charges.tot_amt)
			end
		end
		...
		def compute_charges()
			...
			if facility_charge_exists
				...
				p "in compute Charges"
				p @charge_code
				p gross_total
				@result_vo.charges = compute("C", @charge_code, gross_total)
			end
		end

		def compute_orderitem_taxes()
			order_category_list.each do |one_rec|
				compute_tax(one_rec.tax_code, one_rec.category_total)
				p "computing category_total"
				p order_category_list
				p one_rec.category_total
				p one_rec.tax_code
			end
		end

		def compute_tax(code, amt)
			tax_summary = compute("T", code, amt)

			@all_tax_amt += tax_summary.tot_amt
			@all_tax_comp_list += tax_summary.comp_list
		end
		...
		def get_charge_tax_code()
			...
			facility_rec = FacilityMaster.where(facility_code: facility_id).first
			if facility_rec
				chrg_code = facility_rec.bill_charge_code
				tax_code = facility_rec.bill_charge_tax_code
			end
			p chrg_code, tax_code
